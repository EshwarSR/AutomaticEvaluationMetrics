...
def tokenize_texts(inLines):
    ...
    for doc in inLines:
        ...
        for i in range(2):  # iterate over ref and hyp
            ...
            if WORD_REP == "glove":
                IDs = [[nlp.vocab.strings[t.text.lower()] for t in nlp(sent) if t.text.isalpha(
            ...
            if WORD_REP == "elmo":
                ...
                IDs = [[nlp.vocab.strings[t.text]
                        for t in nlp(sent)] for sent in sent_list]
            id_list = [x for x in IDs if x != []]  # get rid of empty sents
            text_list = [[token.text for token in nlp(
                x)] for x in sent_list if x != []]
    ...
    for i in range(2):
        ...
        for sent_i in range(len(id_doc[i])):
            ...
            if WORD_REP == "elmo":
                ...
                word_emb_list = np.average(sent_vec, axis=0)
                ...
                word_emb_list = [word_emb_list[x] for x in range(
                    len(text_doc[i][sent_i])) if x in keep_inds]
                id_doc[i][sent_i] = [id_doc[i][sent_i][x]
                                     for x in range(len(text_doc[i][sent_i])) if x in keep_inds]
...
def get_weights(id_doc):
    ...
    d_weights = [np.array([], dtype=np.float32),
    ...
    for i in range(len(id_doc)):  # for ref/hyp
        ...
        if METRIC != "wms":
            ...
            d_weights[i] = np.append(d_weights[i], np.array(
...
def calc_smd(input_f, output_f=""):
    ...
    for doc_id in range(len(token_doc_list)):
        ...
        # get D values
        [ref_id_list, hyp_id_list], [
            ref_d, hyp_d] = get_weights([ref_ids, hyp_ids])
