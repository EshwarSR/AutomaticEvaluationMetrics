...
def calculate_similarity(candidate, next_id, emb):
    ...
    similarities = []
    next_id, emb, can_id_list, can_weights = calculator.get_embeddings_ids_weights(can_doc, next_id, emb, method)
    nbow = {
    ...
    # print("NBOW")
    # print(nbow)
    distances = calc.nearest_neighbors("hypothesis", k=len(processed_refs), early_stop=1)
    ...
    print("Time taken for candidate " + str(candidate[id_field]) + " is " + str(time.time() - s))
